...
class kvm_event():
    def __init__(self, wnd_class, wnd_name, server_ip, server_port, logger):
...
def kvm_process(cfg_root_path, type):
    ...
    if wnd_name == 'None':
        wnd_name = None
    ke = kvm_event(wnd_class, wnd_name, server_ip, server_port, logger)
    if type == 'backup':
...
if __name__ == '__main__':
    killOldProcess()
    args = init_args()
    kvm_process(args.config_root, args.type)
